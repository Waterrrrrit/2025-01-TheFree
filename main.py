...
if model_option != "select" or stat_option != "select":
    if model_option != "select":
        ...
        if model_option == "model1":
            ...
            st.subheader("일변수 선형회귀 모델")
            st.image(image_url, caption="URL에서 불러온 이미지", use_container_width=True)
            ...
            st.image(image_url2, caption="URL에서 불러온 이미지", use_container_width=True)
            st.divider()
            ...
            st.subheader("2변수 선형회귀 모델")
            st.image(image_url3, caption="URL에서 불러온 이미지", use_container_width=True)
            ...
            st.image(image_url4, caption="URL에서 불러온 이미지", use_container_width=True)
            st.divider()
            st.subheader("주요 분석")
            ...
            if st.button("개선 방향"):
                st.write("변수 추가를 통해 모델을 개선할 계획입니다. 휘발유 가격 형성에는 복합적인 요인이 작용하므로, 원유 가격, 생산 및 유통 비용, 세금 정책, 국제 정세 등의 변수를 추가적으로 고려할 예정입니다.")   
            ...
            st.subheader("4변수 선형회귀 모델")
            st.image(image_url5, caption="URL에서 불러온 이미지", use_container_width=True)
            st.markdown("본 모델은 **총 4개의 변수(환율, 금 거래가, 유류세, 브렌트유 가격)**를 활용하여 휘발유 가격 예측의 정밀성을 대폭 향상시키기 위해 설계되었습니다.")
            ...
            st.image(image_url6, caption="URL에서 불러온 이미지", use_container_width=True)
            st.divider()
            ...
            if st.button("개선 방향"):
                st.write("모델의 우수한 성능에도 불구하고, 평균 제곱 오차의 증가를 고려해 추가 최적화를 진행할 계획입니다. 변수 간 다중공선성 문제를 점검하고, 비선형 회귀 또는 머신러닝 알고리즘(예: LSTM, GCR)을 도입하여 성능을 더욱 개선할 예정입니다. ")   

        else:
            st.subheader("4변수 딥러닝 모델")
            ...
            st.image(image_url7, caption="URL에서 불러온 이미지", use_container_width=True)
            ...
            st.image(image_url8, caption="URL에서 불러온 이미지", use_container_width=True)
            st.divider()
    ...
    if stat_option != "select":
        ...
        if stat_option == "환율":
            st.write("2020년 1월 1일부터 2023년 12월 31일까지의 환율 변동 추이입니다. 줌/아웃이 가능합니다.")
            ...
            st.write("2020년 1월 1일부터 2023년 12월 31일까지의 금 시세 변동 추이이입니다. 줌/아웃이 가능합니다.")
            ...
            st.write("2020년 1월 1일부터 2023년 12월 31일까지의 국제 유가의 변동 추이입니다. 줌/아웃이 가능합니다.")
            # External data loading
